...
namespace IT_PR_8
{
  public partial class Form1 : Form
  {
    ...
    /// </summary>
    private double XMin = 0.0001;
    /// <summary>
    /// Правая граница графика
    /// </summary>
    private double XMax = Math.PI - 0.0001;
    /// <summary>
    /// Шаг графика
    /// </summary>
    ...
    private void CalcFunction()
    {
      ...
      for (int i = 0; i < count; i++)
      {
        ...
        y1[i] = Convert.ToDouble((0.5 * Math.Pow(x[i], 2) - 1) / Math.Sqrt(x[i]) + (Math.Abs(10 - Math.Pow(Math.E, 0.5 * Math.Pow(x[i], 2)) + 1)) / Math.Log(2 * x[i]) - 1);
       // y2[i] = Math.Cos(x[i]);
      }
    }
